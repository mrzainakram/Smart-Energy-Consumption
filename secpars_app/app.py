...
def _answer_safe(prompt: str, *, use_ctx: bool = False, ctx: str | None = None) -> str:
    try:
        if use_ctx and ctx:
            ...
            full_prompt = prompt
        return answer_text(full_prompt, system=_system(lang_code), prefer=prefer)
    except Exception as e:
        msg = str(e)
        if "429" in msg or "quota" in msg.lower() or "rate" in msg.lower():
            # Try fallback provider
            try:
                return answer_text(full_prompt, system=_system(lang_code), prefer="openai")
            except Exception:
                pass
    ...
    with st.chat_message("assistant"):
        try:
            ...
            def is_project_related_query(query):
                ...
                print(f"DEBUG: Detailed Query Analysis")
                print(f"Full Query: '{query_lower}'")
                print(f"Words: {words}")
                print(f"Keyword Match: {keyword_match}")
                print(f"Word Count: {word_count}")
                print(f"Context Check: {context_checks}")
            ...
            if is_project_query:
                with st.spinner("🔍 Searching in project data..."):
                    ...
                    if rel:
                        ...
                        # Highlight project knowledge source
                        st.markdown(f"""
...
""", unsafe_allow_html=True)
                        ...
                        with st.spinner("🤖 Using Gemini AI (Free)..."):
                            st.info("🌐 Comprehensive AI Answer")
                            ans = _answer_safe(
                                f"User: {user_q}\n\nProvide a helpful, comprehensive answer.",
                                use_ctx=False,
                                prefer="gemini"  # Force Gemini only
                            )
                            st.markdown(f"""
**🌐 Gemini AI Response (Free)**
{ans}

*[Generated by Gemini AI - Always Free]*""", unsafe_allow_html=True)
            else:
                # General or non-project query - Use Gemini only (free)
                with st.spinner("🤖 Using Gemini AI (Free)..."):
                    st.info("🌐 Comprehensive AI Answer")
                    ans = _answer_safe(
                        f"User: {user_q}\n\nProvide a helpful, comprehensive answer.",
...
*[Generated by Gemini AI - Always Free]*""", unsafe_allow_html=True)
            ...
            if "quota" in str(e).lower() or "429" in str(e):
                st.warning("⚠️ OpenAI quota exceeded. Using Gemini AI only.")
                try:
                    ...
                    st.markdown(f"""
**🤖 Gemini AI Response (OpenAI Fallback)**
{ans}

*[Generated by Gemini AI - OpenAI quota exceeded]*""", unsafe_allow_html=True)
                    st.session_state["messages"].append({"role":"assistant","content":ans})
                    st.markdown(ans)
                except Exception as gemini_error:
                    st.error(f"❌ Gemini fallback also failed: {str(gemini_error)}")
